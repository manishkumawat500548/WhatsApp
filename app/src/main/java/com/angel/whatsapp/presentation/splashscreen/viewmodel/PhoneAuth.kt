package com.angel.whatsapp.presentation.splashscreen.viewmodel
...
import com.angel.whatsapp.models.PhoneAuthUser
...
class PhoneAuth @Inject constructor(
    ...
    private fun signWithCredential(
        ...
        firebaseAuth.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                _authState.value = AuthState.Error(task.exception?.message ?: "Sign_in failed")
            }
        }

    }

    private fun markUserAsSignedIn(context: android.content.Context) {
    ...
    fun verifyCode(otp: String,context: android.content.Context){
        ...
        if (currentAuthState !is AuthState.CodeSend || currentAuthState.verificationId.isEmpty()){
            ...
            _authState.value = AuthState.Error("Verification not started")
            return
        }
