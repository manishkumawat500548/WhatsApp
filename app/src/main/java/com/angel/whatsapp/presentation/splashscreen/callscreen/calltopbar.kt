...
@Composable
fun TopBarCall(navController: NavController) {

    var issearching by remember { mutableStateOf(false) }
    var search by remember { mutableStateOf("") }
    var dropdowncall by remember { mutableStateOf(false) }
        ...
        Column() {
            ...
            Row() {
                ...
                if (issearching) {
                    ...
                    IconButton(onClick = { dropdowncall = true }) {
                        Icon(
                        ...
                        DropdownMenu(
                            expanded = dropdowncall,
                            onDismissRequest = { dropdowncall = false },
                            modifier = Modifier.size(width = 170.dp, height = 110.dp)
                        ) {

                            DropdownMenuItem(
                                text = { Text(text = stringResource(R.string.clearcall)) },
                                onClick = { dropdowncall = false })
                            DropdownMenuItem(
                                text = { Text(text = stringResource(R.string.setting)) },
                                onClick = {navController.navigate(Routes.SettingScreenCall)})
